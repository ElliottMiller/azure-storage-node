...
StorageServiceClient.prototype._performRequest = function (webResource, body, options, callback) {
  ...
  if(this.hostType === 'DASH') {
      webResource.withHeader('Expect', '100-continue');
      userAgentComment += "+DASH";
  }
  ...
  var operation = function (options, next) {
    ...
    self._buildRequestOptions(webResource, body, options, function (err, finalRequestOptions) {
      if (err) {
        ...
        if(self.hostType === 'DASH') {
            finalRequestOptions.followAllRedirects = true;
        }
...
/**
 * Determines whether the storage account is a DASH-compliant account
 */
StorageServiceClient.prototype._setStorageHostType = function() {
    
    var webResource = WebResource.get();
    var self = this;
    this._setRequestUrl(webResource, null);
    var response = syncrequest('OPTIONS', webResource.uri);
    if(response && response.headers['x-ms-dash-client'] === 'true') {
        self.hostType = 'DASH';
    } else {
        self.hostType = 'Azure';
    }
